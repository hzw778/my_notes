(function () {
  ...
  var othersLoaded = OTHERS.length > 0;
  var ICONS = {};
  LINKS.groups.forEach(function (g) { g.items.forEach(function (it) { if (it.key) ICONS[it.key] = it.icon; }); });
  LINKS.groups.forEach(function (g) { g.items.forEach(function (it) { if (it.status === "list") OTHER_DEFS.push(it.key); }); });
  function syncOtherKeys() { if (othersLoaded) OTHER_KEYS = OTHERS.map(function (d) { return d.key; }); else OTHER_KEYS = OTHER_DEFS; }
  syncOtherKeys();
  ...
  /* ---------- 本地笔记拆分：mine 聚合数据 → 6 个独立笔记 Tab ---------- */
  var MINE_KEYS = ["mine-paizi", "mine-rag", "mine-mianshi", "mine-agent", "mine-qoder", "mine-jishu"];
  var MINE_SCOPE_MAP = { paizi: "mine-paizi", ragjianli: "mine-rag", qiyemianshi: "mine-mianshi", agentmianjing: "mine-agent", qoder: "mine-qoder", jishu: "mine-jishu" };
  var MINE_TITLES = {
    "mine-paizi": "派聪明知识点（技术派）",
    "mine-rag": "派聪明RAG简历（含面经收集）",
  ...
  (function splitMineNotes() {
    var src = window.TAB_DATA["mine"];
    if (!src || !src.chapters) return;
    var buckets = {};
    MINE_KEYS.forEach(function (k) { buckets[k] = []; });
    src.chapters.forEach(function (c) {
      var key = MINE_SCOPE_MAP[mineScopeOf(c.title)];
      if (key && buckets[key]) buckets[key].push(c);
    });
    MINE_KEYS.forEach(function (k) {
      window.TAB_DATA[k] = { key: k, title: MINE_TITLES[k], url: "", chapters: buckets[k] };
    });
  })();
  ...
  function buildPanels() {
    ...
    if (!wrap) return;
    OTHER_DEFS.forEach(function (key) {
  ...
  function loadOthers() {
    ...
    s.src = "assets/data-others.js";
    s.onload = function () {
      OTHERS = (window.OTHERS || []).filter(function (d) { return !SKIP_OTHERS_KEYS[d.key]; });
      othersLoaded = true;
      syncOtherKeys();
      buildTabbar();
      renderOverview();
      ...
      refreshAll();
      if (state.tab && OTHER_KEYS.indexOf(state.tab) >= 0) switchTab(state.tab);
    };
  ...
  function renderOverview() {
    ...
    LINKS.groups.forEach(function (g) {
      ...
      g.items.forEach(function (it) {
        ...
        var goto = null;
        if (!isPdf && !it.href) {
          if (it.key && DATA[it.key]) goto = it.key;
  ...
  function hashTarget() {
    ...
    if (h && OTHER_KEYS.concat(["overview"]).concat(TAB_ORDER).indexOf(h) >= 0) return h;
    return "overview";
  }
  window.addEventListener("hashchange", function () { switchTab(hashTarget()); });
